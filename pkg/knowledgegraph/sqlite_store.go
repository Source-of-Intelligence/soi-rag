...
// SQLiteGraphStoreConfig SQLite存储配置
type SQLiteGraphStoreConfig struct {
	DBPath string // 数据库文件路径，如 "knowledgegraph.db"、":memory:"（内存模式）
}

// NewSQLiteGraphStore 创建SQLite图存储
func NewSQLiteGraphStore(config SQLiteGraphStoreConfig) (*SQLiteGraphStore, error) {
	if config.DBPath == "" {
		config.DBPath = "knowledgegraph.db"
	}
...
func (s *SQLiteGraphStore) InitSchema(ctx context.Context) error {
	// 实体表
	entityTable := `
...
func (s *SQLiteGraphStore) loadIndexes(ctx context.Context) error {
	// 加载实体名称索引
	...
	for entityRows.Next() {
		...
		if err := entityRows.Scan(&id, &name, &aliasesJSON); err != nil {
...
// AddEntity 添加实体
func (s *SQLiteGraphStore) AddEntity(ctx context.Context, entity *models.Entity) error {
	if entity.ID == "" {
	...
	_, err = s.db.ExecContext(ctx, query,
		...
		entity.Confidence, string(propertiesJSON), entity.SourceDocID, string(aliasesJSON),
		formatTime(time.Now()),
	)
	if err != nil {
...
// AddRelation 添加关系
func (s *SQLiteGraphStore) AddRelation(ctx context.Context, relation *models.Relation) error {
	if relation.ID == "" {
...
// GetEntity 获取实体
func (s *SQLiteGraphStore) GetEntity(ctx context.Context, id string) (*models.Entity, error) {
	query := `
	...
	var entityType, propertiesJSON, aliasesJSON string

	err := s.db.QueryRowContext(ctx, query, id).Scan(
		&entity.ID, &entity.Name, &entityType, &entity.Description,
		&entity.Confidence, &propertiesJSON, &entity.SourceDocID, &aliasesJSON,
	)
...
// GetRelation 获取关系
func (s *SQLiteGraphStore) GetRelation(ctx context.Context, id string) (*models.Relation, error) {
	query := `
	...
	var relationType, propertiesJSON string
	...
	if err := json.Unmarshal([]byte(propertiesJSON), &relation.Properties); err != nil {
...
// SearchEntities 搜索实体
func (s *SQLiteGraphStore) SearchEntities(ctx context.Context, query string, limit int) ([]*models.Entity, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
// QueryRelations 查询关系
func (s *SQLiteGraphStore) QueryRelations(ctx context.Context, entityID string, relationType models.RelationType) ([]*models.Relation, error) {
	s.mu.RLock()
...
// GetSubgraph 获取子图
func (s *SQLiteGraphStore) GetSubgraph(ctx context.Context, entityIDs []string, depth int) (*models.Subgraph, error) {
	if depth <= 0 {
		depth = 2
	}
...
// FindPath 查找路径
func (s *SQLiteGraphStore) FindPath(ctx context.Context, sourceID, targetID string, maxDepth int) ([]*models.Path, error) {
	if maxDepth <= 0 {
		maxDepth = 5
	}
...
func (s *SQLiteGraphStore) DeleteByDocument(ctx context.Context, docID string) error {
	// 获取要删除的实体和关系
	var entityIDs []string
	entityRows, err := s.db.QueryContext(ctx, "SELECT id, name, aliases FROM entities WHERE document_id = ?", docID)
	if err != nil {
		return fmt.Errorf("查询实体失败: %w", err)
	}
	defer entityRows.Close()
	...
	for entityRows.Next() {
		var id, name, aliasesJSON string
		if err := entityRows.Scan(&id, &name, &aliasesJSON); err != nil {
			continue
		}
	...
	relationRows, err := s.db.QueryContext(ctx, "SELECT id, source_id, target_id FROM relations WHERE document_id = ?", docID)
	if err != nil {
		return fmt.Errorf("查询关系失败: %w", err)
	}
	defer relationRows.Close()
	...
	// 删除数据库中的数据
	if _, err := s.db.ExecContext(ctx, "DELETE FROM relations WHERE document_id = ?", docID); err != nil {
		return fmt.Errorf("删除关系失败: %w", err)
	}

	if _, err := s.db.ExecContext(ctx, "DELETE FROM entities WHERE document_id = ?", docID); err != nil {
